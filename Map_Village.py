...
class MapVillage: # フィールド画面クラス
    def __init__(self, screen): # 初期化
        ...
        # 背景を一枚絵で置き換える画像
        self.bg_image = load_image("fig/2.png")
    ...
    def draw(self): # 描画処理
        ...
        # 背景画像がある場合はマップ全体を一枚画像で描画（カメラオフセット対応）
        if self.bg_image:
            full_bg = pygame.transform.scale(self.bg_image, (map_width, map_height))
            self.screen.blit(full_bg, (-camera_x, -camera_y))
        else:
            # マップ描画（タイルごと）
